...
// Tipos de notificaciones que consideramos "de tienda"
...
export function StoreNotificationBell() {
    ...
    // Filtrar notificaciones de tienda
    // Por ahora, como no tenemos tipos específicos definidos en el sistema backend para tienda,
    // esto actuará como un placeholder o filtrará si existieran.
    // Si no hay tipos específicos, podríamos mostrar una lista vacía o simular.
    const storeNotifications = notifications.filter(n => STORE_NOTIFICATION_TYPES.includes(n.type));

    // Calcular no leídas de tienda
    ...
    return (
        <>
            <div className="relative">
                <button
                    ...
                    className={`relative -m-2.5 p-2.5 text-gray-400 transition-colors duration-200 ${theme === 'light'
                        ? 'hover:text-gray-500'
                        : 'hover:text-blue-400' // Different hover color for store
                        }`}
                ...
                {showDropdown && (
                    <div className={`absolute right-0 mt-2 w-80 rounded-lg shadow-2xl border backdrop-blur-xl z-50 max-h-96 overflow-hidden ${theme === 'light'
                        ...
                        : 'bg-dark-bg-secondary border-blue-400/30'
                        }`}>
                        <div className={`p-4 border-b flex items-center justify-between ${theme === 'light'
                            ...
                            <div>
                                <h3 className={`text-lg font-semibold flex items-center gap-2 ${theme === 'light'
                                    ? 'text-gray-900'
                                    : 'text-white'
                                    }`}>
                        ...
                        <div className="max-h-80 overflow-y-auto">
                            {storeNotifications.length === 0 ? (
                                <div className={`p-6 text-center ${theme === 'light'
                                    ...
                                    <p>No hay notificaciones de tienda</p>
                                </div>
                            ) : (
                                    ...
                                    {storeNotifications.slice(0, 5).map((notification) => (
                                        <button
                                            ...
                                            className={`w-full p-4 text-left transition-colors ${theme === 'light'
                                                ? `hover:bg-gray-50 ${!notification.isRead ? 'bg-blue-50' : ''}`
                                                : `hover:bg-dark-bg-tertiary/50 ${!notification.isRead ? 'bg-blue-400/5 border-l-2 border-blue-400' : ''}`
                                                }`}
                                        >
            ...
            {/* Reutilizamos el modal de notificaciones, asumiendo que sirve para cualquier tipo */}
            <NotificationModal
